...
import me.crashcringle.jobsplus.customenchanting.CustomEnchantment.EnchantmentSetOnItem;
...
public class CustomEnchantListener implements Listener {
	...
	public void onBlockBreak(BlockBreakEvent event) {
		...
		if (custEnch.hasCustomEnchantmentType(inhandEnchs, "Molten Touch")) {
			...
			if (block.getType() == Material.IRON_ORE) {
				...
				world.dropItemNaturally(loc, new ItemStack(Material.TERRACOTTA));
			} else if (block.getType() == Material.CACTUS) {
				Tools.removeNearbyEntityDelay(loc, Material.CACTUS, 1.3, 1L);
				world.dropItemNaturally(loc, new ItemStack(Material.INK_SAC, 1));
			}
		}
	...
	public void onBucketFill(PlayerBucketFillEvent event) {
		...
		List<EnchantmentSetOnItem> inhandEnchs = custEnch.getCustomEnchantments(bucket);
			...
			if (custEnch.hasCustomEnchantmentType(inhandEnchs, "Hydration")
				...
				for (int x = radius * -1; x < radius; x++) {
					for (int z = radius * -1; z < radius; z++) {
						...
						if (tblock.getType() == Material.FARMLAND
								&& MonstersPlus.isBuildLocation(player, tblock.getLocation())) {
							tblock.setBlockData(Material.FARMLAND.createBlockData());
						}
					}
				}
			}
